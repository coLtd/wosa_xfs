  ...
  methods: {
    ...
    fetchMenu () {
      ...
      this.$http.get('/api').then(({ data }) => {
        ...
        keys.map(key => {
          list.push(Object.assign(data[key], { title: key, href: `/${data[key]['Provider']}` }))
        })
        this.deviceReduce(list)
        this.constructMenu()
        this.$store.commit('setMenu', this.menus)
      })
    },
    deviceReduce (data) {
      this.deviceTypes = data.reduce((res, item) => {
        res.hasOwnProperty(item['Class']) ? res[item['Class']] : (res[item['Class']] = [])
        res[item['Class']].push(item)
        return res
      }, {})
    },
    constructMenu () {
      this.menus = [{ href: '/', title: 'Home', icon: 'home' }]
      let types = Object.keys(this.deviceTypes)
      types.map(type => {
        this.menus.push(Object.assign({ title: type, icon: 'domain' }, { items: this.deviceTypes[type] }))
      })
